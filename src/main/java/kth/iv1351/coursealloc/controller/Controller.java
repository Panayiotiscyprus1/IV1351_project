...
public class Controller {
     ...
     *   A teacher is not allowed to teach in more than 4 course instances in the same period (same study_year + study_period).
     */
    public void allocateTeaching(String instanceId,
        ...
        try {
            ...
            // Get target instance year & period (CRUD)
            InstancePeriod ip = db.getInstancePeriod(instanceId);

            // Check if teacher already has *any* allocation on this instance
            boolean alreadyOnThisInstance = db.teacherAlreadyAllocatedOnInstance(instanceId, employmentId);

            // If this is a new instance for that teacher in that period, enforce the "max 4 instances per period" rule
            ...
            // All good -> do the actual allocation
            ...
            // we already rolled back above but do it again for safety
            db.rollback();
            throw e;
        } catch (SQLException e) {
            db.rollback();
    ...
    /**
     * Ddeallocate a teaching activity for a teacher on an instance.
